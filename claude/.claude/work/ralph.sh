...
4. If it's a UI task, check that the UI looks correct and functions correctly with /agent-browser skill. \
...
After completing the task, read tasks.json and check if every entry has passes set to true. If so: output <promise>COMPLETE</promise>. \
")
  echo "$result"
